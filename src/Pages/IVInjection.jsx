...
const IVInjection = () => {
  ...
  // Safely get IV Injection nurses or empty array if not available
  const ivNurses = nurses?.ivinjection || [];

  useEffect(() => {
    handleSort('rating');
  }, []);

  // Service pricing information specific to IV Injection
  const servicePricing = {
    basic: {
      name: "Basic IV Therapy",
      price: "₹120/injection",
      description: "Standard IV medication administration"
    },
    advanced: {
      name: "IV Hydration Therapy",
      price: "₹575/5injection",
      description: "Includes vitamin cocktails and electrolytes"
    },
    specialty: {
      name: "Specialty IV Treatments",
      price: "₹1150/10injection",
      description: "For chemotherapy or complex medications"
    }
  };

  const handleSort = (criteria) => {
    setSortBy(criteria);
    const sorted = [...ivNurses].sort((a, b) => {
      switch (criteria) {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = ivNurses.filter(nurse => 
  ...
  const handleFilter = (filter) => {
    ...
    if (filter === 'available') {
      ...
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Pediatric IV Specialist'));
    } else if (filter === 'chemo') {
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Chemotherapy Certified'));
    }
    ...
    >
      ...
      >
        <div className="max-w-3xl relative z-10">
          <div className="flex items-center mb-3">
            <h1 className="text-3xl font-bold text-gray-900 mr-3">IV Injection & Therapy Services</h1>
        ...
        <div className="absolute right-0 top-0 h-full opacity-20">
          <svg className="h-full w-auto" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <line x1="100" y1="50" x2="100" y2="150" stroke="#4F46E5" strokeWidth="8"/>
          </svg>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="text-2xl font-bold text-blue-600">{ivNurses.filter(n => n.available_now).length}</div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          <div className="text-sm text-gray-500">Patient Rating</div>
          <div className="text-2xl font-bold text-yellow-600">4.97/5</div>
        </div>
      </motion.div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center">
            ...
            <div className="hidden sm:flex space-x-2">
              ...
              <button
                onClick={() => handleFilter('chemo')}
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'chemo' ? 'bg-pink-100 text-pink-800' : 'bg-gray-100 text-gray-700'}`}
              >
                Chemotherapy
              </button>
            </div>
        ...
        <AnimatePresence>
          {filteredNurses.length > 0 ? (
              ...
              >
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="mb-3">
                      <p className="text-xs text-gray-500 mb-1">IV Specializations</p>
                  ...
                  {nurse.certifications?.includes('Pediatric IV Specialist') && (
                  ...
                  {nurse.certifications?.includes('Chemotherapy Certified') && (
            ...
            >
              ...
              <h3 className="text-lg font-medium text-gray-700 mb-2">No IV specialists available</h3>
              <p className="text-gray-500 mb-6">We couldn't find any IV injection nurses matching your criteria</p>
              <motion.button
