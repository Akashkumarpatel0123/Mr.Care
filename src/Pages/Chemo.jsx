...
const Chemo = () => {
  ...
  const [selectedService, setSelectedService] = useState('portCare');

  // Safely get chemo nurses or empty array if not available
  const chemoNurses = nurses?.chemo || [];
  ...
  const servicePricing = {
    portCare: {
      name: "Chemo Port Care",
      price: "₹1300",
      description: "Professional chemotherapy port care and maintenance"
    }
  };

  const handleSort = (criteria) => {
    setSortBy(criteria);
    const sorted = [...chemoNurses].sort((a, b) => {
      switch (criteria) {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = chemoNurses.filter(nurse => 
  ...
  const handleFilter = (filter) => {
    ...
    if (filter === 'available') {
      ...
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Oncology Specialist'));
    }
    ...
    >
      ...
      >
        <div className="max-w-3xl relative z-10">
          <div className="flex items-center mb-3">
            <h1 className="text-3xl font-bold text-gray-900 mr-3">At-Home Chemo Port Services</h1>
          ...
          {/* Service Pricing - Single Option */}
          <div className="mb-4 p-4 bg-white rounded-lg border border-green-200 shadow-sm">
            <div className="flex justify-between items-center">
              <div>
                <h3 className="text-lg font-medium text-gray-900">Chemo Port Care</h3>
                <p className="text-sm text-gray-600">Professional chemotherapy port care and maintenance</p>
              </div>
              <div className="text-2xl font-bold text-green-600">₹1300</div>
            </div>
          </div>
        ...
        <div className="absolute right-0 top-0 h-full opacity-20">
          <svg className="h-full w-auto" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M50 100L150 100M100 50L100 150M85 85L115 115M115 85L85 115" stroke="#10B981" strokeWidth="8" strokeLinecap="round"/>
          </svg>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="text-2xl font-bold text-blue-600">{chemoNurses.filter(n => n.available_now).length}</div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
      ...
      >
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center">
            <h2 className="text-lg font-semibold text-gray-800 mr-4">
              {filteredNurses.length} {filteredNurses.length === 1 ? 'Oncology Nurse Available' : 'Oncology Nurses Available'}
            </h2>
            ...
            <div className="hidden sm:flex space-x-2">
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'fast' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-700'}`}
              >
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center w-full md:w-auto gap-3">
            ...
            <div className="flex items-center space-x-2 w-full md:w-auto">
              ...
              >
                ...
                <option value="price_asc">Price: Low to High (Daily)</option>
                <option value="price_desc">Price: High to Low (Daily)</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="sm:hidden mt-4 flex space-x-2 overflow-x-auto pb-2">
          ...
          <button
            onClick={() => handleFilter('fast')}
            className={`px-3 py-1 rounded-full text-sm whitespace-nowrap ${activeFilter === 'fast' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-700'}`}
          >
            Fastest
          </button>
          <button
        ...
        <AnimatePresence>
          {filteredNurses.length > 0 ? (
            filteredNurses.map((nurse, index) => (
              ...
              >
                ...
                >
                  ...
                  <div className="p-4">
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium">₹{nurse.daily_rate || 'N/A'}/day</p>
                      </div>
                      <div>
                    ...
                    <div className="mb-3 p-2 bg-gray-50 rounded-lg">
                      ...
                      <p className="font-medium">Chemo Port Care</p>
                      <p className="text-sm text-green-600 font-bold">₹1300</p>
                    </div>
                  ...
                  {/* Certification Badge */}
                  {nurse.certifications?.includes('Oncology Specialist') && (
            ...
            >
              ...
              <h3 className="text-lg font-medium text-gray-700 mb-2">No oncology nurses available</h3>
              <p className="text-gray-500 mb-6">We couldn't find any chemo port specialists matching your criteria</p>
              <motion.button
