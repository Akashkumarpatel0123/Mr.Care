...
const Vaccination = () => {
  ...
  // Safely get vaccination nurses or empty array if not available
  const vaccinationNurses = nurses?.vaccination || [];

  useEffect(() => {
    handleSort('rating');
  }, []);

  // Service pricing information specific to Vaccination
  const servicePricing = {
    routine: {
      name: "Routine Vaccination",
      price: "₹120",
      description: "Standard immunization services (flu, tetanus, etc.)"
    },
  ...
  const handleSort = (criteria) => {
    setSortBy(criteria);
    const sorted = [...vaccinationNurses].sort((a, b) => {
      switch (criteria) {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = vaccinationNurses.filter(nurse => 
  ...
  const handleFilter = (filter) => {
    ...
    let filtered = [...vaccinationNurses];
    
    if (filter === 'available') {
      filtered = filtered.filter(nurse => nurse.available_now);
    } else if (filter === 'pediatric') {
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Pediatric Vaccination Specialist'));
    } else if (filter === 'travel') {
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Travel Vaccination Certified'));
    } else if (filter === 'covid') {
      filtered = filtered.filter(nurse => nurse.certifications?.includes('COVID-19 Vaccination Certified'));
    }
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="mb-8 bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-2xl border border-gray-100 relative overflow-hidden"
      >
        <div className="max-w-3xl relative z-10">
          <div className="flex items-center mb-3">
            <h1 className="text-3xl font-bold text-gray-900 mr-3">Vaccination & Immunization Services</h1>
          ...
          <div className="mb-4">
            <h3 className="text-lg font-medium text-gray-800 mb-2">Select Vaccination Type:</h3>
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-lg font-bold text-indigo-700">{service.price}</span>
                    </div>
        ...
        {/* Decorative syringe illustration */}
        <div className="absolute right-0 top-0 h-full opacity-20">
          <svg className="h-full w-auto" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M100 50L100 150" stroke="#6366F1" strokeWidth="8" strokeLinecap="round"/>
            <path d="M80 70L120 70" stroke="#6366F1" strokeWidth="8" strokeLinecap="round"/>
            <path d="M90 130L110 130" stroke="#6366F1" strokeWidth="8" strokeLinecap="round"/>
            <path d="M70 100L130 100" stroke="#6366F1" strokeWidth="8" strokeLinecap="round"/>
          </svg>
        </div>
      </motion.div>
      ...
      >
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="text-2xl font-bold text-indigo-600">30 min</div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="text-2xl font-bold text-blue-600">{vaccinationNurses.filter(n => n.available_now).length}</div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          <div className="text-sm text-gray-500">Patient Rating</div>
          <div className="text-2xl font-bold text-yellow-600">4.95/5</div>
        </div>
      </motion.div>
      ...
      >
          ...
          <div className="flex items-center">
            ...
            <div className="hidden sm:flex space-x-2">
              <button
                ...
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'all' ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-700'}`}
              >
                All
              </button>
              <button
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'pediatric' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-700'}`}
              >
                Pediatric Specialists
              </button>
              <button
                onClick={() => handleFilter('travel')}
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'travel' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-700'}`}
              >
                Travel Vaccines
              </button>
              <button
                onClick={() => handleFilter('covid')}
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'covid' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-700'}`}
              >
                COVID-19 Certified
              </button>
            </div>
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center w-full md:w-auto gap-3">
            ...
            <div className="relative w-full md:w-64">
              ...
              <input
                ...
                className="border rounded-lg pl-10 pr-3 py-2 w-full text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
              />
            </div>
        ...
        <div className="sm:hidden mt-4 flex space-x-2 overflow-x-auto pb-2">
          <button
            onClick={() => handleFilter('all')}
            className={`px-3 py-1 rounded-full text-sm whitespace-nowrap ${activeFilter === 'all' ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-700'}`}
          >
            All
          </button>
          <button
          ...
          <button
            onClick={() => handleFilter('pediatric')}
            className={`px-3 py-1 rounded-full text-sm whitespace-nowrap ${activeFilter === 'pediatric' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-700'}`}
          >
            Pediatric
          </button>
          <button
            onClick={() => handleFilter('travel')}
            className={`px-3 py-1 rounded-full text-sm whitespace-nowrap ${activeFilter === 'travel' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-700'}`}
          >
            Travel
          </button>
        </div>
        ...
        <AnimatePresence>
          {filteredNurses.length > 0 ? (
              ...
              >
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="mb-3">
                      <p className="text-xs text-gray-500 mb-1">Vaccination Certifications</p>
                      <div className="flex flex-wrap gap-2">
                        {nurse.certifications?.map((cert, i) => (
                          <span key={i} className="bg-indigo-50 text-indigo-800 px-2 py-1 rounded text-xs">
                            {cert}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="mb-3 p-2 bg-gray-50 rounded-lg">
                      ...
                      <p className="text-sm text-indigo-600 font-bold">{servicePricing[selectedService].price}</p>
                    </div>
                  ...
                  >
                    ...
                    `}>
                      ...
                      <span>
                        {hoveredStatus === index
                          ? nurse.available_now 
                            ? `Available - ${nurse.distance < 5 ? 'Can arrive in 20 min' : 'Can arrive in 35 min'}` 
                  ...
                  {nurse.available_now && (
                    <div className="absolute top-14 right-3 z-10">
                      <div className="flex items-center bg-blue-50 px-2.5 py-1 rounded-full text-xs font-medium text-blue-800 border border-blue-200 shadow-xs">
                        ...
                        {nurse.distance < 5 ? '20 min' : '35 min'}
                      </div>
                    </div>
                  )}

                  {/* Certification Badges */}
                  {nurse.certifications?.includes('Pediatric Vaccination Specialist') && (
                  ...
                  {nurse.certifications?.includes('Travel Vaccination Certified') && (
            ...
            >
              ...
              <h3 className="text-lg font-medium text-gray-700 mb-2">No vaccination specialists available</h3>
              <p className="text-gray-500 mb-6">We couldn't find any immunization nurses matching your criteria</p>
              <motion.button
                ...
                className="px-5 py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 shadow-sm"
              >
                Reset All Filters
              </motion.button>
            </motion.div>
