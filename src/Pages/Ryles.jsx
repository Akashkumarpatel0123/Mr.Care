...
const RylesTube = () => {
  ...
  const [selectedService, setSelectedService] = useState('insertion');

  // Safely get Ryles tube nurses or empty array if not available
  const rylesNurses = nurses?.ryles || [];

  useEffect(() => {
    handleSort('rating');
  }, []);

  // Service pricing information specific to Ryles Tube
  const servicePricing = {
    insertion: {
      name: "Standard Insertion",
      price: "₹350",
      description: "Nasogastric tube placement and verification"
    },
  ...
  const handleSort = (criteria) => {
    setSortBy(criteria);
    const sorted = [...rylesNurses].sort((a, b) => {
      switch (criteria) {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = rylesNurses.filter(nurse => 
  ...
  const handleFilter = (filter) => {
    ...
    if (filter === 'available') {
      ...
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Pediatric NG Specialist'));
    } else if (filter === 'critical') {
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Critical Care NG'));
    }
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="mb-8 bg-gradient-to-r from-blue-50 to-teal-50 p-6 rounded-2xl border border-gray-100 relative overflow-hidden"
      >
        <div className="max-w-3xl relative z-10">
          <div className="flex items-center mb-3">
            <h1 className="text-3xl font-bold text-gray-900 mr-3">Ryle's Tube Specialists</h1>
        ...
        <div className="absolute right-0 top-0 h-full opacity-20">
          <svg className="h-full w-auto" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M100 50L150 100L100 150L50 100L100 50Z" stroke="#0D9488" strokeWidth="8"/>
            <line x1="100" y1="50" x2="100" y2="150" stroke="#0D9488" strokeWidth="8"/>
            <circle cx="100" cy="100" r="20" stroke="#0D9488" strokeWidth="8"/>
          </svg>
        </div>
      </motion.div>
      ...
      >
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="text-2xl font-bold text-teal-600">40 min</div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="text-2xl font-bold text-blue-600">{rylesNurses.filter(n => n.available_now).length}</div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
      ...
      >
          ...
          <div className="flex items-center">
            ...
            <div className="hidden sm:flex space-x-2">
              <button
                ...
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'all' ? 'bg-teal-100 text-teal-800' : 'bg-gray-100 text-gray-700'}`}
              >
                All
              </button>
              <button
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'urgent' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-700'}`}
              >
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center w-full md:w-auto gap-3">
            ...
            <div className="relative w-full md:w-64">
              ...
              <input
                ...
                className="border rounded-lg pl-10 pr-3 py-2 w-full text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
              />
            </div>
            ...
            <div className="flex items-center space-x-2 w-full md:w-auto">
              ...
              <select
                ...
                className="border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500 bg-white"
              >
        ...
        <div className="sm:hidden mt-4 flex space-x-2 overflow-x-auto pb-2">
          <button
            onClick={() => handleFilter('all')}
            className={`px-3 py-1 rounded-full text-sm whitespace-nowrap ${activeFilter === 'all' ? 'bg-teal-100 text-teal-800' : 'bg-gray-100 text-gray-700'}`}
          >
            All
          </button>
          <button
          ...
          <button
            onClick={() => handleFilter('urgent')}
            className={`px-3 py-1 rounded-full text-sm whitespace-nowrap ${activeFilter === 'urgent' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-700'}`}
          >
            Urgent
          </button>
          <button
        ...
        <AnimatePresence>
          {filteredNurses.length > 0 ? (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="mb-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {nurse.certifications?.map((cert, i) => (
                          <span key={i} className="bg-teal-50 text-teal-800 px-2 py-1 rounded text-xs">
                            {cert}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="mb-3 p-2 bg-gray-50 rounded-lg">
                      ...
                      <p className="text-sm text-teal-600 font-bold">{servicePricing[selectedService].price}</p>
                    </div>
                  ...
                  {nurse.certifications?.includes('Pediatric NG Specialist') && (
                  ...
                  {nurse.certifications?.includes('Critical Care NG') && (
            ...
            >
              ...
              <h3 className="text-lg font-medium text-gray-700 mb-2">No Ryle's tube specialists available</h3>
              ...
              <motion.button
                ...
                className="px-5 py-2.5 bg-teal-600 text-white rounded-lg text-sm font-medium hover:bg-teal-700 shadow-sm"
              >
                Reset All Filters
              </motion.button>
            </motion.div>
