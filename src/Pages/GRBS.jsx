...
const GRBS = () => {
  ...
  // Safely get GRBS nurses or empty array if not available
  const grbsNurses = nurses?.grbs || [];
  ...
  const servicePricing = {
    standard: {
      name: "Standard GRBS Test",
      price: "₹120",
      description: "Single blood sugar reading "
    },
  ...
  const handleSort = (criteria) => {
    setSortBy(criteria);
    const sorted = [...grbsNurses].sort((a, b) => {
      switch (criteria) {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = grbsNurses.filter(nurse => 
  ...
  const handleFilter = (filter) => {
    ...
    if (filter === 'available') {
      ...
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Diabetes Care Specialist'));
    }
    ...
    >
      ...
      >
        <div className="max-w-3xl relative z-10">
          <div className="flex items-center mb-3">
            <h1 className="text-3xl font-bold text-gray-900 mr-3">At-Home Blood Sugar Testing</h1>
          ...
          <div className="mb-4">
            <h3 className="text-lg font-medium text-gray-800 mb-2">Select Test Type:</h3>
        ...
        <div className="absolute right-0 top-0 h-full opacity-20">
          <svg className="h-full w-auto" viewBox="0 0 200 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M20 50H40M40 50L60 30M60 30L80 70M80 70L100 30M100 30L120 70M120 70L140 30M140 30L160 50M160 50H180" stroke="#4F46E5" strokeWidth="2" strokeLinecap="round"/>
            <circle cx="100" cy="50" r="15" fill="#4F46E5" fillOpacity="0.2"/>
          </svg>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="text-2xl font-bold text-blue-600">{grbsNurses.filter(n => n.available_now).length}</div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
      ...
      >
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center">
            ...
            <div className="hidden sm:flex space-x-2">
              ...
              <button
                onClick={() => handleFilter('diabetes')}
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'diabetes' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-700'}`}
              >
                Diabetes Specialist
              </button>
            </div>
        ...
        <div className="sm:hidden mt-4 flex space-x-2 overflow-x-auto pb-2">
          ...
          <button
            onClick={() => handleFilter('diabetes')}
            className={`px-3 py-1 rounded-full text-sm whitespace-nowrap ${activeFilter === 'diabetes' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-700'}`}
          >
            Diabetes
          </button>
        </div>
            ...
            filteredNurses.map((nurse, index) => (
              ...
              >
                ...
                >
                  ...
                  {/* Certification Badge */}
                  {nurse.certifications?.includes('Diabetes Care Specialist') && (
