...
const Colostomy = () => {
  ...
  // Safely get colostomy nurses or empty array if not available
  const colostomyNurses = nurses?.colostomy || [];
  ...
  const servicePricing = {
    basic: {
      name: "Basic Colostomy Change",
      price: "₹450",
      description: "Standard colostomy bag replacement with basic supplies"
    },
  ...
  const handleSort = (criteria) => {
    setSortBy(criteria);
    const sorted = [...colostomyNurses].sort((a, b) => {
      switch (criteria) {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = colostomyNurses.filter(nurse => 
  ...
  const handleFilter = (filter) => {
    ...
    let filtered = [...colostomyNurses];
    
    if (filter === 'available') {
      filtered = filtered.filter(nurse => nurse.available_now);
    } else if (filter === 'wound') {
      filtered = filtered.filter(nurse => nurse.skills?.includes('Wound Care Specialist'));
    } else if (filter === 'certified') {
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Ostomy Care Certified'));
    }
    ...
    >
      ...
      >
        <div className="max-w-3xl relative z-10">
          <div className="flex items-center mb-3">
            <h1 className="text-3xl font-bold text-gray-900 mr-3">At-Home Colostomy Care</h1>
        ...
        <div className="absolute right-0 top-0 h-full opacity-20">
          <svg className="h-full w-auto" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M50 100L150 100M100 50L100 150M85 85L115 115M115 85L85 115" stroke="#3B82F6" strokeWidth="8" strokeLinecap="round"/>
          </svg>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="text-2xl font-bold text-teal-600">{colostomyNurses.filter(n => n.available_now).length}</div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
      ...
      >
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center">
            ...
            <div className="hidden sm:flex space-x-2">
              ...
              <button
                onClick={() => handleFilter('wound')}
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'wound' ? 'bg-teal-100 text-teal-800' : 'bg-gray-100 text-gray-700'}`}
              >
                Wound Care Experts
              </button>
              <button
        ...
        <div className="sm:hidden mt-4 flex space-x-2 overflow-x-auto pb-2">
          ...
          <button
            onClick={() => handleFilter('wound')}
            className={`px-3 py-1 rounded-full text-sm whitespace-nowrap ${activeFilter === 'wound' ? 'bg-teal-100 text-teal-800' : 'bg-gray-100 text-gray-700'}`}
          >
            Wound Care
          </button>
          <button
                  ...
                  >
                    ...
                    `}>
                      ...
                      <span>
                        {hoveredStatus === index
                          ? nurse.available_now 
                            ? `Available - ${nurse.distance < 5 ? 'Can arrive in 45 min' : 'Can arrive in 60 min'}` 
                  ...
                  {nurse.available_now && (
                    <div className="absolute top-14 right-3 z-10">
                      <div className="flex items-center bg-blue-50 px-2.5 py-1 rounded-full text-xs font-medium text-blue-800 border border-blue-200 shadow-xs">
                        ...
                        {nurse.distance < 5 ? '45 min' : '60 min'}
                      </div>
                    </div>
                  )}

                  {/* Certification Badge */}
                  {nurse.certifications?.includes('Ostomy Care Certified') && (
