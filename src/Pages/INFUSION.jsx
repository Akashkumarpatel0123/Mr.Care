...
const Infusion = () => {
  ...
  // Safely get Infusion nurses or empty array if not available
  const infusionNurses = nurses?.infusion || [];

  useEffect(() => {
    handleSort('rating');
  }, []);

  // Service pricing information specific to Infusion
  const servicePricing = {
    standard: {
      name: "Standard Infusion",
      price: "₹500/hour",
      description: "Basic IV infusion with pre-established cannula"
    },
    extended: {
      name: "Extended Infusion",
      price: "₹2500/6hours",
      description: "Long duration infusions with monitoring"
    },
    specialty: {
      name: "Specialty Infusions",
      price: "₹4000/8hours",
      description: "For complex medications or blood products"
    }
  };

  const handleSort = (criteria) => {
    setSortBy(criteria);
    const sorted = [...infusionNurses].sort((a, b) => {
      switch (criteria) {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = infusionNurses.filter(nurse => 
  ...
  const handleFilter = (filter) => {
    ...
    if (filter === 'available') {
      ...
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Pediatric Infusion'));
    } else if (filter === 'critical') {
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Critical Care'));
    }
    ...
    >
      ...
      >
        <div className="max-w-3xl relative z-10">
          <div className="flex items-center mb-3">
            <h1 className="text-3xl font-bold text-gray-900 mr-3">IV Infusion Services</h1>
      ...
      >
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="text-2xl font-bold text-blue-600">{infusionNurses.filter(n => n.available_now).length}</div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          <div className="text-sm text-gray-500">Patient Rating</div>
          <div className="text-2xl font-bold text-yellow-600">4.98/5</div>
        </div>
      </motion.div>
          ...
          <div className="flex items-center">
            ...
            <div className="hidden sm:flex space-x-2">
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'critical' ? 'bg-pink-100 text-pink-800' : 'bg-gray-100 text-gray-700'}`}
              >
                Critical Care
              </button>
            </div>
        ...
        <AnimatePresence>
          {filteredNurses.length > 0 ? (
              ...
              >
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="mb-3">
                      <p className="text-xs text-gray-500 mb-1">Infusion Specializations</p>
                  ...
                  {nurse.certifications?.includes('Pediatric Infusion') && (
                  ...
                  {nurse.certifications?.includes('Critical Care') && (
            ...
            >
              ...
              <h3 className="text-lg font-medium text-gray-700 mb-2">No infusion specialists available</h3>
              <p className="text-gray-500 mb-6">We couldn't find any infusion nurses matching your criteria</p>
              <motion.button
