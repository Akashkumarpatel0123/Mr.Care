...
const Foley = () => {
  ...
  // Safely get Foley nurses or empty array if not available
  const foleyNurses = nurses?.foley || [];
  ...
  const servicePricing = {
    insertion: {
      ...
      price: "₹550",
      description: "Sterile insertion with all necessary supplies"
    },
  ...
  const handleSort = (criteria) => {
    setSortBy(criteria);
    const sorted = [...foleyNurses].sort((a, b) => {
      switch (criteria) {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = foleyNurses.filter(nurse => 
  ...
  const handleFilter = (filter) => {
    ...
    if (filter === 'available') {
      ...
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Urology Specialist'));
    }
    ...
    >
      ...
      >
        <div className="max-w-3xl relative z-10">
          <div className="flex items-center mb-3">
            <h1 className="text-3xl font-bold text-gray-900 mr-3">Foley Catheter Services</h1>
        ...
        <div className="absolute right-0 top-0 h-full opacity-20">
          <svg className="h-full w-auto" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M50 50L150 150M50 150L150 50" stroke="#4F46E5" strokeWidth="8" strokeLinecap="round"/>
      ...
      >
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="text-2xl font-bold text-blue-600">{foleyNurses.filter(n => n.available_now).length}</div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
      ...
      >
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center">
            ...
            <div className="hidden sm:flex space-x-2">
              ...
              <button
                onClick={() => handleFilter('urology')}
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'urology' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-700'}`}
              >
                Urology Certified
              </button>
            </div>
        ...
        <div className="sm:hidden mt-4 flex space-x-2 overflow-x-auto pb-2">
          ...
          <button
            onClick={() => handleFilter('urology')}
            className={`px-3 py-1 rounded-full text-sm whitespace-nowrap ${activeFilter === 'urology' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-700'}`}
          >
            Urology
          </button>
        </div>
            ...
            filteredNurses.map((nurse, index) => (
              ...
              >
                ...
                >
                  ...
                  {/* Certification Badge */}
                  {nurse.certifications?.includes('Urology Specialist') && (
