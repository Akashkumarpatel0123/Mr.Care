...
const ECG = () => {
  ...
  // Safely get ECG nurses or empty array if not available
  const ecgNurses = nurses?.ecg || [];
  ...
  const servicePricing = {
    standard: {
      name: "Standard ECG",
      price: "₹550",
      description: "Basic 12-lead ECG with report"
    },
  ...
  const handleSort = (criteria) => {
    setSortBy(criteria);
    const sorted = [...ecgNurses].sort((a, b) => {
      switch (criteria) {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = ecgNurses.filter(nurse => 
  ...
  const handleFilter = (filter) => {
    ...
    let filtered = [...ecgNurses];
    
    if (filter === 'available') {
      filtered = filtered.filter(nurse => nurse.available_now);
    } else if (filter === 'fast') {
      filtered = filtered.filter(nurse => nurse.available_now && nurse.distance < 5);
    } else if (filter === 'cardiology') {
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Cardiology Specialist'));
    }
    ...
    >
      ...
      >
        <div className="max-w-3xl relative z-10">
          <div className="flex items-center mb-3">
            <h1 className="text-3xl font-bold text-gray-900 mr-3">At-Home ECG Services</h1>
        ...
        <div className="absolute right-0 top-0 h-full opacity-20">
          <svg className="h-full w-auto" viewBox="0 0 200 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M10 50H30M30 50L50 30M50 30L70 70M70 70L90 30M90 30L110 70M110 70L130 30M130 30L150 50M150 50H170" stroke="#4F46E5" strokeWidth="2" strokeLinecap="round"/>
          </svg>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="text-2xl font-bold text-blue-600">{ecgNurses.filter(n => n.available_now).length}</div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
      ...
      >
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center">
            ...
            <div className="hidden sm:flex space-x-2">
              ...
              <button
                onClick={() => handleFilter('cardiology')}
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'cardiology' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-700'}`}
              >
        ...
        <div className="sm:hidden mt-4 flex space-x-2 overflow-x-auto pb-2">
          ...
          <button
            onClick={() => handleFilter('cardiology')}
            className={`px-3 py-1 rounded-full text-sm whitespace-nowrap ${activeFilter === 'cardiology' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-700'}`}
          >
            Cardiology
          </button>
        </div>
            ...
            filteredNurses.map((nurse, index) => (
              ...
              >
                ...
                >
                  ...
                  {/* Certification Badge */}
                  {nurse.certifications?.includes('Cardiology Specialist') && (
