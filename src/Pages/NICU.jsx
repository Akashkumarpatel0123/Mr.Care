...
const NICU = () => {
  ...
  // Safely get NICU nurses or empty array if not available
  const nicuNurses = nurses?.nicu || [];

  useEffect(() => {
    handleSort('rating');
  }, []);

  // Service pricing information specific to NICU
  const servicePricing = {
    basic: {
      name: "Basic NICU Care",
      price: "₹1500/12hour",
      description: "Standard neonatal monitoring and care"
    },
    advanced: {
      name: "Advanced NICU Care",
      price: "₹7300/5day",
      description: "Includes preemie care and feeding support"
    },
    critical: {
      name: "Critical Neonatal Care",
      price: "₹14800/10day",
      description: "For high-risk infants with specialized needs"
    }
  };

  const handleSort = (criteria) => {
    setSortBy(criteria);
    const sorted = [...nicuNurses].sort((a, b) => {
      switch (criteria) {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = nicuNurses.filter(nurse => 
  ...
  const handleFilter = (filter) => {
    ...
    } else if (filter === 'preemie') {
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Preemie Specialist'));
    } else if (filter === 'lactation') {
      filtered = filtered.filter(nurse => nurse.certifications?.includes('Lactation Consultant'));
    }
    ...
    >
      ...
      >
        <div className="max-w-3xl relative z-10">
          <div className="flex items-center mb-3">
            <h1 className="text-3xl font-bold text-gray-900 mr-3">Neonatal Intensive Care (NICU) Specialists</h1>
          ...
          <div className="mb-4">
            <h3 className="text-lg font-medium text-gray-800 mb-2">Select Care Level:</h3>
        ...
        <div className="absolute right-0 top-0 h-full opacity-20">
          <svg className="h-full w-auto" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M100 50C122.091 50 140 67.9086 140 90C140 112.091 122.091 130 100 130C77.9086 130 60 112.091 60 90C60 67.9086 77.9086 50 100 50Z" fill="#A78BFA"/>
            <path d="M100 150C111.046 150 120 141.046 120 130C120 118.954 111.046 110 100 110C88.9543 110 80 118.954 80 130C80 141.046 88.9543 150 100 150Z" fill="#A78BFA"/>
          </svg>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="text-2xl font-bold text-blue-600">{nicuNurses.filter(n => n.available_now).length}</div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
      ...
      >
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center">
            ...
            <div className="hidden sm:flex space-x-2">
              ...
              <button
                onClick={() => handleFilter('preemie')}
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'preemie' ? 'bg-pink-100 text-pink-800' : 'bg-gray-100 text-gray-700'}`}
              >
                Preemie Care
              </button>
              <button
                onClick={() => handleFilter('lactation')}
                className={`px-3 py-1 rounded-full text-sm ${activeFilter === 'lactation' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-700'}`}
              >
                Lactation Support
              </button>
            </div>
        ...
        <div className="sm:hidden mt-4 flex space-x-2 overflow-x-auto pb-2">
          ...
          <button
            onClick={() => handleFilter('preemie')}
            className={`px-3 py-1 rounded-full text-sm whitespace-nowrap ${activeFilter === 'preemie' ? 'bg-pink-100 text-pink-800' : 'bg-gray-100 text-gray-700'}`}
          >
            Preemie
          </button>
        </div>
        ...
        <AnimatePresence>
          {filteredNurses.length > 0 ? (
            filteredNurses.map((nurse, index) => (
              ...
              >
                ...
                >
                  ...
                  {nurse.certifications?.includes('Preemie Specialist') && (
                    <div className="absolute top-14 left-3 z-10">
                      <div className="flex items-center bg-pink-50 px-2.5 py-1 rounded-full text-xs font-medium text-pink-800 border border-pink-200 shadow-xs">
                        <svg className="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  {nurse.certifications?.includes('Lactation Consultant') && (
            ...
            >
              ...
              <h3 className="text-lg font-medium text-gray-700 mb-2">No NICU specialists available</h3>
              <p className="text-gray-500 mb-6">We couldn't find any neonatal nurses matching your criteria</p>
              <motion.button
